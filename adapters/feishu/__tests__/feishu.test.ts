...
describe('Feishu: project picker card', () => {
  const sampleProjects: RecentProject[] = [
    {
      projectPath: '/Users/dev/aranya',
      realPath: '/Users/dev/aranya',
      projectName: 'aranya',
      isGit: true,
      repoName: 'aranya',
      branch: 'main',
  ...
  it('info column has title markdown + notation path markdown', () => {
    ...
    expect(info[0].content).toContain('**aranya**')
    ...
    expect(info[1].content).toContain('aranya')
  })
  ...
  it('row button says 选择 with small size and carries per-project value', () => {
    ...
    expect(btn1.value.realPath).toBe('/Users/dev/aranya')
    expect(btn1.value.projectName).toBe('aranya')
...
describe('Feishu: card.action.trigger parsing', () => {
  ...
  it('parses pick_project action from event', () => {
    ...
    expect(event.action.value.realPath).toBe('/Users/dev/aranya')
    expect(event.action.value.projectName).toBe('aranya')
  })
