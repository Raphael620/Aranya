...
describe('ConversationService', () => {
  ...
  test('buildChildEnv injects desktop Computer Use host bundle id for sdk sessions', async () => {
    ...
    expect(env.ARANYA_COMPUTER_USE_HOST_BUNDLE_ID).toBe(
      'com.aranya.desktop',
    )
    expect(env.ARANYA_DESKTOP_SERVER_URL).toBe('http://127.0.0.1:3456')
  ...
  test('buildChildEnv asks desktop SDK sessions to wait briefly for MCP tools', async () => {
    ...
    expect(env.ARANYA_DESKTOP_AWAIT_MCP).toBe('1')
    expect(env.ARANYA_DESKTOP_AWAIT_MCP_TIMEOUT_MS).toBe('5000')
  })
