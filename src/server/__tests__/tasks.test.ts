...
describe('TaskService', () => {
  ...
  it('should list tasks from JSON files', async () => {
    const listDir = path.join(tmpDir, 'tasks', 'default')
    await fs.mkdir(listDir, { recursive: true })

    await fs.writeFile(path.join(listDir, 'task-001.json'), JSON.stringify({
      id: 'task-001',
      subject: 'code-review',
      status: 'completed',
    ...
    await fs.writeFile(path.join(listDir, 'task-002.json'), JSON.stringify({
      id: 'task-002',
  ...
  it('should get single task by ID', async () => {
    ...
    await fs.writeFile(path.join(listDir, 'abc.json'), JSON.stringify({
      id: 'abc',
      subject: 'build',
      status: 'completed',
    }))
  ...
  it('should skip invalid JSON files gracefully', async () => {
    const listDir = path.join(tmpDir, 'tasks', 'default')
    await fs.mkdir(listDir, { recursive: true })
    await fs.writeFile(path.join(listDir, 'bad.json'), 'not json {{{')
...
describe('Tasks API', () => {
  ...
  it('should return tasks when files exist', async () => {
    const listDir = path.join(tmpDir, 'tasks', 'default')
    await fs.mkdir(listDir, { recursive: true })
    await fs.writeFile(path.join(listDir, 'test.json'), JSON.stringify({
