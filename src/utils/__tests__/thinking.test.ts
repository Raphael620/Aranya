...
describe('provider-aware thinking support', () => {
  ...
  beforeEach(() => {
    ...
    originalExplicitDisabledThinking = process.env.ARANYA_SEND_DISABLED_THINKING
  ...
  afterEach(() => {
    ...
    restoreEnv('ARANYA_SEND_DISABLED_THINKING', originalExplicitDisabledThinking)
  ...
  test('only sends explicit disabled thinking when the provider opts in', () => {
    delete process.env.ARANYA_SEND_DISABLED_THINKING
    expect(shouldSendExplicitDisabledThinking()).toBe(false)

    process.env.ARANYA_SEND_DISABLED_THINKING = '1'
